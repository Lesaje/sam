...
Video::Video(std::string _img_path, int class_num) : image_path(_img_path)
{
...
void Video::thread_for_read()
{
    read_thread = std::thread(&Video::readImage, this);
}

cv::Size Video::getWindowSize()
{
    return window_size;
}

void Video::setImageQueue(std::shared_ptr<MessageQueue<cv::Mat>> _image_queue)
{
    image_queue = _image_queue;
}

void Video::setDetectionQueue(std::shared_ptr<MessageQueue<cv::Mat>> _detect_queue)
{
...
int Video::getDetectFreq()
{
    return _detect_freq;
}

// Draw the result of detection on image(reference parameter)
void Video::drawResult(cv::Mat &image,
